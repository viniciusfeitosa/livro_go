...
func (so Soma) Calcular() int {
	return so.operando1 + so.operando2
}

func (so Soma) String() string {
	return fmt.Sprintf("%d - %d", so.operando1, so.operando2)
}
...
func main() {
	...
	fmt.Println("Idade acumuladas =", acumular(idades))
}
